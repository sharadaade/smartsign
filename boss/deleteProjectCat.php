...
//print_r($_GET);die;
	...
	$sql = "SELECT cat_id,project_id,project_image
	        FROM tbl_project
	...
	$result = dbQuery($sql);
		...
		while ($row = dbFetchAssoc($result)) {
			...
			//echo $row[0];die;
				...
				if($cntCatId>1)
				{
				  $sqlupdate =mysqli_query("UPDATE tbl_project_category SET cat_id = replace(cat_id, '".$categories[$i].",', '') WHERE FIND_IN_SET('".$categories[$i]."',cat_id)>0");
				...
				elseif($cntCatId== 1)
				{
					...
					// delete the products
					$sqlD = "DELETE FROM tbl_project_category
