...
//print_r($_GET);die;
	...
	$sql = "SELECT cat_id,service_id,service_image
	        FROM tbl_service
	...
	$result = dbQuery($sql);
		...
		while ($row = dbFetchAssoc($result)) {
			...
			//echo $row[0];die;
				...
				if($cntCatId>1)
				{
				  $sqlupdate =mysqli_query("UPDATE tbl_service_category SET cat_id = replace(cat_id, '".$categories[$i].",', '') WHERE FIND_IN_SET('".$categories[$i]."',cat_id)>0");

				}
				elseif($cntCatId== 1)
				{
					$image_list = explode(',',$row['service_image']);
					...
					foreach($image_list as $row2)
					{
						...
						$new_image_list = '';
						unlink('../service/big_img/'.$row2);
						unlink('../service/'.$row2);
					}
					
					// delete the products
					$sqlD = "DELETE FROM tbl_service_category
